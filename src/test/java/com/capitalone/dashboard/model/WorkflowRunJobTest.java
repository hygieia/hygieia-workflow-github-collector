...
public class WorkflowRunJobTest {
	...
	private List<GitWorkflowRunJobStep> steps;
	private GitWorkflowRunJobStep step1;
	private GitWorkflowRunJobStep step2;
	private GitWorkflowRunJobStep step3;
	private String stepNumber = "001";
	private String stepNumber2 = "002";
	
	@Before
	public void setup() {
		steps = new ArrayList<GitWorkflowRunJobStep>();
		
		step1 = new GitWorkflowRunJobStep(workflowId,runId,jobId,stepNumber,status,conclusion,startedAt,completedAt,name);
		steps.add(step1);

		step2 = new GitWorkflowRunJobStep(workflowId,runId,jobId,stepNumber,status,conclusion,startedAt,completedAt,name);
		steps.add(step2);

		step3 = new GitWorkflowRunJobStep(workflowId,runId,jobId,stepNumber,status,conclusion,startedAt,completedAt,name);
	...
	public void testConstructorWithSetters() {
		GitWorkflowRunJob wfrj = new GitWorkflowRunJob(workflowId, runId, jobId, status, conclusion, startedAt, completedAt, name);
		...
		List<GitWorkflowRunJobStep> steps1 = wfrj.getWorkflowRunJobSteps();
		
		int i = 0;
		GitWorkflowRunJobStep stepOne = null;
		GitWorkflowRunJobStep stepTwo = null;
		for (GitWorkflowRunJobStep step : steps1) {
			if (i == 0) {
				stepOne = step;
			}
			else if (i == 1) {
